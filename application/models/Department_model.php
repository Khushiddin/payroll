...
class Department_model extends CI_Model
{
    ...
    {
        $this->db->select('id, departmentName, createdDtm');
        $this->db->from('tbl_departments');
        if(!empty($searchText)) {
            $likeCriteria = "(departmentName  LIKE '%".$searchText."%')";
     ...
     */
    function departmentListing($searchText = '', $page, $segment)
    {
        $this->db->select('id, departmentName, createdDtm');
        $this->db->from('tbl_departments');
        if(!empty($searchText)) {
     ...
     */
    function addNewDepartment($userInfo)
    {
        $this->db->trans_start();
        $this->db->insert('tbl_departments', $userInfo);
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_departments', $departmentInfo);
     ...
     */
    function deleteDepartment($id, $departmentInfo)
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_departments', $departmentInfo);
