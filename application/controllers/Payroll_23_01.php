        ...
        if($payrollId){
            ...
            if(!empty($data)){
                foreach($data as $user){
                    ...
                    $pDays = $user->presentDays;
                            $days = cal_days_in_month(CAL_GREGORIAN, $user->month, $user->year);
                            $naration = 'Pay Slip for '.$user->month.'-'.$user->year;
            ...
            $A_gross = $basic+$spclAllow+$lta+$hra+$bonus+$user->arrear;
            ...
            $vpf = 0;
//             echo $record->basic.',';
//         echo $record->transAllow.',';
//         echo $record->spclAllow.',';
//         echo $record->lta.',';
//         echo $record->hra.',';
// print_r($A_gross_withoutBonus);die();
        $A_epf = (12/100)*($basic);
        if($A_gross_withoutBonus <= 21000){
            $esic = (0.75/100)*($A_gross+$user->incentive+$user->rimbersement);
        }else{
            $esic = 0;
        }
        $deduction = $A_epf+$esic+$user->tds+$user->PT+$user->advance_deduction+$user->arrear_pf+$user->arrear_esic;
